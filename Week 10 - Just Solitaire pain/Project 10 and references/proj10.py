    ...
    #  Computer Project #10
    #  Algorithm
    #    initialize the board using funciton
    #    prnt menu

    #    loop while not end-of-data
    #       prompt for an input of the option
    #       check option using parse option funciton
    #       check if option is none and continue if it is
    #       index into the option to get prompt inputs
    #       if option is TT
    #           check if called cuntion returns false
    #           if false
    #               print invalid move
    #       elif option is TF
    #           check if called cuntion returns false
    #           if false
    #               print invalid move
    #           check if check win funciton returns true
    #           if true
    #               print you won and display winning board
    #               break
    #
    #       elif option is WT
    #           check if called cuntion returns false
...
MENU ='''Prompt the user for an option and check that the input has the 
    ...
    '''
def initialize():
    '''The function has no parameters and returns the starting state of the game with the four data structures, tableau, stock, foundation, waste '''
    stock = Deck()
    tableau = [[], [], [], [], [], [], []]
    foundation = [[], [], [], []]
    waste = []
    ...
    try:
        ...
        try:
            ...
            if tab_card.rank() == 1 and foundation[f_num] == []:
                ...
                if not tableau[t_num][-1].is_face_up():
                    ...
                    return True
                # if tableau card is face up pop and return true
        ...
        if tab_card.rank() == 1 and foundation[f_num] == []:
            ...
            if not tableau[t_num][-1].is_face_up():
                tableau[t_num][-1].flip_card()
                return True
            # if tableau card is face up return true
            if tableau[t_num][-1].is_face_up():
                return True
...
def main():   
    ...
    while True:
        ...
        # if option is none then continue
        if option == None:
            continue
        opt = option[0]
